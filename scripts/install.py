...
def upsert_marked_block(path: Path, title: str, body: str) -> None:
...
def install_codex(source: Path, scope: str, project: Path) -> Path:
...
def install_claude(scope: str, project: Path) -> Path:
    if scope == "global":
    ...
    upsert_marked_block(path, "HTML Presentation Skill", bridge_body(skill_path))
    return path


def install_copilot(scope: str, project: Path) -> Path:
    if scope == "global":
    ...
    upsert_marked_block(path, "HTML Presentation Skill", bridge_body(f".codex/skills/{SKILL_NAME}"))
    return path


def install_antigravity(scope: str, project: Path) -> Path:
    if scope == "global":
...
def main() -> int:
    ...
    if "codex" in requested or requested & {"claude", "copilot", "antigravity"}:
        target = install_codex(source, args.scope, project)
        installed.append(f"Codex skill files -> {target}")

    for agent in sorted(requested - {"codex"}):
        try:
            if agent == "claude":
                installed.append(f"Claude instructions -> {install_claude(args.scope, project)}")
            elif agent == "copilot":
                installed.append(f"Copilot instructions -> {install_copilot(args.scope, project)}")
            elif agent == "antigravity":
                installed.append(f"Antigravity/generic agent instructions -> {install_antigravity(args.scope, project)}")
            else:
